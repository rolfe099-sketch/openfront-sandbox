...
## Phase 1K Build Availability Preflight V2

OpenFront Sandbox now activates two narrow source-derived placement/target blockers in `src/core/build/build-availability-preflight.ts` using the Phase 1J map surface context.

Implemented scope:

- `warship-target-not-water`: active only for `Warship`, only when `targetTile` and `mapSurfaceContext` are provided, and only after the target ref is valid. This derives from the first guard in `PlayerImpl.warshipSpawn`.
- `mirv-target-has-no-owner`: active only for `MIRV`, only when `targetTile` and `mapSurfaceContext` are provided, and only after the target ref is valid. This derives from the MIRV branch in `PlayerImpl.canSpawnUnitType`.

Invalid target refs short-circuit before surface or ownership predicates are read. The passing status remains `available-for-future-checks`, and summaries continue to state that the result is not full OpenFront build legality.

This does not implement full placement legality, full `canSpawnUnitType`, `validStructureSpawnTiles`, port placement, `structureMinDist`, `radiusPortSpawn`, transport ship placement, trade ship targeting, nuke/silo targeting beyond the MIRV owner guard, costs, gold checks, player alive state, spawn phase logic, upgrade validity, construction duration, construction execution, combat, scoring, simulation, UI canvas, extension behavior, browser permissions, content scripts, or OpenFront integration.
...
| Build availability preflight boundaries | `src/core/game/GameMap.ts`; `src/core/game/PlayerImpl.ts`; `src/core/execution/ConstructionExecution.ts`; `src/core/execution/ExecutionManager.ts`; `src/core/GameRunner.ts`; `src/core/worker/WorkerMessages.ts`; `src/core/configuration/Config.ts`; `src/core/Schemas.ts`; `src/core/game/Game.ts` | implemented for listed scope | Source-aligned preflight model is implemented in `src/core/build/build-availability-preflight.ts` with tests. Active checks are limited to player-buildable catalogue membership, explicit disabled-unit membership, target ref bounds, Warship target-water guard, and MIRV target-owner guard. Future reason categories remain metadata only. | Implemented for preflight V2 boundary checks only. |
| Placement boundary map | `src/core/game/GameMap.ts`; `src/core/game/PlayerImpl.ts`; `src/core/game/TransportShipUtils.ts`; `src/core/execution/ConstructionExecution.ts`; `src/core/configuration/Config.ts` | partially implemented for listed scope | Placement boundary categories are documented in `src/core/build/placement-boundary-map.ts` with tests. Ownership and shoreline/coast/water categories are partially active only for the Phase 1K MIRV owner and Warship water blockers; all broader placement categories remain future-only. | Implemented for two narrow blockers only; not full placement legality. |
...
| Nukes/MIRV | `src/core/execution/NukeExecution.ts`; `src/core/execution/MIRVExecution.ts`; `src/core/configuration/Config.ts`; `src/core/game/PlayerImpl.ts`; `src/client/render/gl/utils/NukeTrajectory.ts`; `src/client/render/frame/derive/NukeTelegraphs.ts`; `src/client/render/gl/passes/NukeTrajectoryPass.ts`; `src/client/render/gl/passes/NukeTelegraphPass.ts`; `src/client/render/gl/shaders/nuke-trajectory/**`; `src/client/render/gl/shaders/nuke-telegraph/**` | partially implemented for listed scope | Phase 1K implements only the MIRV target has-owner preflight guard from `PlayerImpl.canSpawnUnitType`. Nuke execution, silo selection, spawn immunity, teammate rules, cooldowns, MIRV execution, and rendering paths remain source-located future work. | Implemented for MIRV owner preflight guard only. |
| Warships | `src/core/execution/WarshipExecution.ts`; `src/core/execution/MoveWarshipExecution.ts`; `src/core/execution/ShellExecution.ts`; `src/core/configuration/Config.ts`; `src/core/game/PlayerImpl.ts`; `src/core/execution/nation/NationWarshipBehavior.ts`; `src/client/controllers/WarshipSelectionController.ts` | partially implemented for listed scope | Phase 1K implements only the Warship target-water preflight guard from `PlayerImpl.warshipSpawn`. Warship state, movement, targeting, shells, port interactions, water components, nation behavior, and client selection references remain source-located future work. | Implemented for Warship target-water preflight guard only. |
