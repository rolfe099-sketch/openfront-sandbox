...
| Build/action catalogue and grouping | `src/core/game/Game.ts` symbols `UnitType`, `Nukes`, `Structures`, `BuildableAttacks`, `BuildMenus`, `PlayerBuildable`, `BuildableUnit`; `src/core/execution/ConstructionExecution.ts` symbol `ConstructionExecution`; `src/core/game/PlayerImpl.ts` symbols `buildableUnits`, `canBuild`, `canSpawnUnitType`; `src/client/hud/layers/BuildMenu.ts` symbol `buildTable`; `src/core/execution/ExecutionManager.ts` symbol `intentToExecution` | `af2849a2d71a7700a72c077a9e5616e990e584f6` | 2026-06-11 | implemented | source-code verified | source-derived behavior | Implemented in `src/core/build/build-action-catalogue.ts` with tests in `src/core/build/build-action-catalogue.test.ts`. Scope is limited to source-derived unit groups, build menu display order, and construction kind classification. Costs, placement legality, disabled-unit checks, economy, construction duration, combat, execution behavior, UI icons/assets, scoring, and simulation are not implemented. |
